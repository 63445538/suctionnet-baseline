...
def create_dense_point_cloud(model_root, save_root):
    ...
    **Output:**
    ...
    for model in models:
        ...
        # v, f, n = pcu.load_mesh_vfn(mesh_dir)
        v_poisson, n_poisson = pcu.sample_mesh_poisson_disk(v, f, num_samples=-1, radius=0.0002, use_geodesic_distance=True)

        save_file = os.path.join(save_dir, model+'.npz')
        np.savez(save_file, points=v_poisson, normals=n_poisson)


model_path = '/media/rcao/Data/Dataset/graspnet/models'
save_root = '/media/rcao/Data/Dataset/graspnet/dense_point_clouds'

create_dense_point_cloud(model_path, save_root)
