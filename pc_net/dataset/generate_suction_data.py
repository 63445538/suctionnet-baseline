...
def generate_scene(scene_id, cfgs):
    ...
    save_path_root = os.path.join(dataset_root, 'suction_' + data_type)
    os.makedirs(save_path_root, exist_ok=True)
    for ann_id in range(256):
        # get scene point cloud
        print('generating {} scene: {} ann: {}'.format(data_type, scene_id, ann_id))
        if data_type == 'real':
            ...
            rgb = np.array(Image.open(os.path.join(dataset_root, 'virtual_scenes', 'scene_' + str(scene_id).zfill(4),
                                                camera_type, str(ann_id).zfill(4) + '_rgb.png')))
            depth = np.array(Image.open(os.path.join(dataset_root, 'virtual_scenes', 'scene_' + str(scene_id).zfill(4),
                                                    camera_type, str(ann_id).zfill(4) + '_depth.png')))
            seg = np.array(Image.open(os.path.join(dataset_root, 'virtual_scenes', 'scene_' + str(scene_id).zfill(4),
                                                camera_type, str(ann_id).zfill(4) + '_label.png')))
        else:
        ...
        cloud_masked = torch.from_numpy(cloud_masked).cuda().float()
        grasp_points = grasp_points.contiguous().unsqueeze(0)
        cloud_masked = cloud_masked.contiguous().unsqueeze(0)
        _, nn_inds, _ = knn_points(cloud_masked, grasp_points, K=1)
        nn_inds = nn_inds.squeeze(-1).squeeze(0)
        cloud_masked_seal_scores = torch.index_select(grasp_points_seal, 0, nn_inds).cpu().numpy()
        cloud_masked_wrench_scores = torch.index_select(grasp_points_wrench, 0, nn_inds).cpu().numpy()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_type', default='syn', help='[real/syn]')
    cfgs = parser.parse_args()
    
    parallel_generate(list(range(130)), cfgs=cfgs, proc = 20)