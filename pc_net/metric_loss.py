...
from cuda_ops_knn.pointops.functions import pointops
...
def negative_loglikelihood(muA, muP, muN, varA, varP, varN, margin=0.0):
    ...
    T1 = varP2 + 2 * muP2 * varP + 2 * (varA + muA2) * (
                varP + muP2) - 2 * muA2 * muP2 - 4 * muA * muP * varP  # ([2047, 5])
    T2 = varN2 + 2 * muN2 * varN + 2 * (varA + muA2) * (
                varN + muN2) - 2 * muA2 * muN2 - 4 * muA * muN * varN  # ([2047, 5])
...
def kl_div_gauss(mu_q, var_q, mu_p, var_p):  # (D, N), (1, N)
    ...
    kl = 0.5 * ((var_q / var_p) * D + 1.0 / (var_p) * torch.sum(mu_p ** 2 + mu_q ** 2 - 2 * mu_p * mu_q, axis=0) - D
                + D * (torch.log(var_p) - torch.log(var_q)))
...
class MetricLoss(nn.Module):
    ...
    def __init__(self, nsample=36, varPrior=1/96.0, kl_scale_factor=1e-6, margin=0, epsilon=0.1):
        super().__init__()
        self.bayesian_triplet_loss = BayesianTripletLoss(varPrior=varPrior, kl_scale_factor=kl_scale_factor,
                                                         margin=margin)
    ...
    def forward(self, feature, sigma, xyz, label):
        ...
        neighbor_feature = feature[neighbor_idx.view(-1).long(), :].view(m, nsample,
                                                                         feature.shape[1])  # ([m, nsample, c])
        ...
        posmask = torch.abs(label.repeat((1, nsample)) - neighbor_label.view(m, nsample)) < self.epsilon
        point_mask = torch.sum(posmask.int(), -1)  # (m)
        point_mask = torch.logical_and(0 < point_mask, point_mask < nsample)

        # mean_point = torch.mean(point_mask.float())
        # pos_num = torch.nonzero(point_mask).size()[0]
        # print("eps:{}, mean_num:{}, pos_num:{}".format(self.epsilon, mean_point.item(), pos_num))
        ...
        # test1 = torch.arange(posmask.shape[0]).flatten()
        # test2 = torch.randint(low=0, high=posmask.shape[1], size=(posmask.shape[0], 1)).flatten()
        # test3 = torch.stack((test1, test2)).T

        # TODO: choose more than 1 sample from neighbors
        pos_inds = torch.stack((torch.arange(posmask.shape[0]).flatten(),
                                torch.randint(low=0, high=posmask.shape[1], size=(posmask.shape[0], 1)).flatten())).T
        pos_feats = neighbor_feature[pos_inds[:, 0], pos_inds[:, 1], :]
        pos_sigma = neighbor_sigma[pos_inds[:, 0], pos_inds[:, 1], :]

        neg_inds = torch.stack((torch.arange(posmask.shape[0]).flatten(),
                                torch.randint(low=0, high=posmask.shape[1], size=(posmask.shape[0], 1)).flatten())).T
        ...
        loss, probs, t_mu, t_sigma2 = self.bayesian_triplet_loss(feature[tri_inds], pos_feats[tri_inds],
                                                                 neg_feats[tri_inds], sigma[tri_inds],
                                                                 pos_sigma[tri_inds], neg_sigma[tri_inds])
